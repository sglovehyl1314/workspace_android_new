...
import com.nari.sun.view.Block;
...
public class MinesweeperGame extends Activity {
	...
	private TextView txtTimer; // 计时器
	private ImageButton btnSmile;// 笑脸
	private TextView txtMineCount;// 已找出的雷数
	...
	private void showMineField() {
		...
		for (int row = 1; row < numberOfRowsInMineField + 1; row++) {
			...
			for (int column = 1; column < numberOfColumnsInMineField + 1; column++) {
				blocks[row][column].setLayoutParams(new LayoutParams(blockDimension + 2 * blockPadding, blockDimension + 2 * blockPadding));
				blocks[row][column].setPadding(blockPadding, blockPadding, blockPadding, blockPadding);
				tableRow.addView(blocks[row][column]);
			}
			mineField.addView(tableRow, new TableLayout.LayoutParams((blockDimension + 2 * blockPadding) * numberOfColumnsInMineField, blockDimension + 2 * blockPadding));
		}
	}
	...
	private void rippleUncover(int rowClicked, int columnClicked) {
		...
		for (int row = 0; row < 3; row++) {
			for (int column = 0; column < 3; column++) {
				...
				if (blocks[rowClicked + row - 1][columnClicked + column - 1].isCovered() && (rowClicked + row - 1 > 0) && (columnClicked + column - 1 > 0)
